...
} from './enhanced-nlp-improved.js';
import { 
  applyVisualization 
} from './visualization-integration.js';
import { visualizeLocations } from './visualization.js';

// We need to set a valid Mapbox token for the map to load properly
let map;
let mapboxToken;
let selectedVisualizationType = 'default'; // Default visualization type
...
function handleProcessedResult(result) {
  ...
  displayMessage(result.message || `I found these locations mentioned: ${locationNames.join(', ')}`);
  
  // Display location chips
  displayLocationChipsImproved(result.locations, messageDisplay);
...
/**
 * Display location chips with improved styling and functionality
 * @param {Array} locations - Array of location objects
 * @param {HTMLElement} container - Container to display the chips in
 */
function displayLocationChipsImproved(locations, container) {
...
function initVisualizationOptions() {
  ...
  vizOptions.forEach(option => {
    option.addEventListener('click', () => {
      ...
      if (messageContainer.style.display !== 'none' && messageContainer.querySelectorAll('.location-chip').length > 0) {
        // Get the current input text
        const inputValue = document.getElementById('nlp-input').value;
        if (inputValue) {
          // Re-process the input with the new visualization type
          processInput(inputValue, selectedVisualizationType);
        }
      }
    });
  });
...
async function processInput(inputText, visualizationType) {
  if (!inputText) return;
  ...
  // Append visualization type if not already specified in the input
  const enhancedInput = 
    inputText.toLowerCase().includes(visualizationType.toLowerCase()) ?
    inputText : `${inputText} (visualize as ${visualizationType})`;
  
  try {
    // Process the input with Gemini API through our enhanced NLP module
    const result = await processNaturalLanguageInput(enhancedInput);
    
    // Explicitly set the visualization type
    ...
    console.error('Error processing input:', error);
    loadingIndicator.style.display = 'none';
    displayMessage('Error processing your request. Please try again.');
  }
} 